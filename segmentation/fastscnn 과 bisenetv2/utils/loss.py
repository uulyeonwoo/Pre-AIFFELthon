    ...
    def _aux_forward(self, *inputs, **kwargs):
        *preds, target = tuple(inputs)
        ...
        for i in range(1, len(preds)):
            aux_size = preds[i].size()[2:]
            masks_aux = F.interpolate(target, aux_size, mode='nearest')
            masks_aux = masks_aux.squeeze(1).long()
            aux_loss = super(MixSoftmaxCrossEntropyLoss, self
                                                    ).forward(preds[i], masks_aux)
    ...
    def _aux_forward(self, *inputs, **kwargs):
        *preds, target = tuple(inputs)
        ...
        for i in range(1, len(preds)):
            ...
            masks_aux = F.interpolate(target, aux_size, mode='nearest')
            masks_aux = masks_aux.squeeze(1).long()
            aux_loss = super(MixSoftmaxCrossEntropyOHEMLoss, self
                                                    ).forward(preds[i], masks_aux)
            loss += self.aux_weight * aux_loss
        return loss
