...
-> CityScape 데이터 설명에 클래스 수가 30개로 나와 있었으나 체크포인트 모델은 19개인 상황

chk_pnt_path = "../bisenetv2-aux_zh320.pth"
chk_pnt_map = torch.load(chk_pnt_path, map_location='cpu')

print(chk_pnt_map["state_dict"]['detail_branch.0.0.weight'])
...
# 모델 모드 변경; 추론 모드, CUDA 사용
model.eval()
model.cuda()

# 아래 코드는 다음 레포에서 가져옴; github.com/CoinCheung/BiSeNet
import math
import torch
import torch.nn as nn
import torch.nn.functional as F
...
! ln -s "~/aiffel/aiffelthon/bisenetv2_Cheung/BiSeNet/test1.jpg" ./test1.jpg
